  ...
  try {
    ...
    if (question_no === 0 && bit_string === "0") {
      ...
      if (firstQuestionData) {
        ...
        return res.json({
          message: "Question generated successfully",
          output: getQuestionResponse(1, firstQuestionData, '1')
        });
      } else {
        return res.status(404).json({ message: "First question data not found" });
      }
    ...
    while (nextQuestionData) {
      if (!checkCriteria(updatedBitString, nextQuestionData.criteria, nextQuestionData.criteriaOr)) {
        ...
        return res.json({
          message: "Next question is available based on your responses.",
          output: getQuestionResponse(nextQuestionNo, nextQuestionData, updatedBitString)
        });
      }
    }
...
// Helper function to get consistent question response format
function getQuestionResponse(questionNo, questionData, bitString) {
  return {
